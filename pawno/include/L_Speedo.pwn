...
#define VehicleMaxSpeed  50000000                //Define the Speed Limit! (KPH)
#define SpeedoLogoText   "~b~~h~M~r~e~y~~h~G~p~~h~a  ~g~~h~~h~F~b~~h~r~r~~h~~h~e~y~e~p~R~r~~h~~h~o~b~a~g~~h~~h~M" //Define Speedo Logo (Text in Speedo Top)
...
public LuX_SpeedoMeterUp()
{
	...
	{
	LVehicleStatus = "~r~Cerrado";
	}
	else
	{
	LVehicleStatus = "~g~Abierto";
	}
	...
	format(lstring,sizeof(lstring),"~b~Vehiculo: ~w~%s~n~~b~Vida: ~w~%.2f~n~~b~Altitud: ~w~%.1f~n~~b~Ubicación: ~w~%s~n~~b~Estado: %s",LVehiclesName[GetVehicleModel(GetPlayerVehicleID(i))-400],L_VehHealth,LPosZ,LuxZone,LVehicleStatus);
	...
	if(L_VehHealth <= 500){
	format(lstring,sizeof(lstring),"%sVehiculo: ~w~%s~n~%sVida: ~w~~r~%.2f~n~%sAltitud: ~w~%.1f~n~%sUbicación: ~w~%s",CategoriesColor,LVehiclesName[GetVehicleModel(GetPlayerVehicleID(i))-400],CategoriesColor,L_VehHealth,CategoriesColor,LPosZ,CategoriesColor,LuxZone);
	TextDrawSetString(LFunc[i], lstring);
    }
...
public OnPlayerCommandText(playerid,cmdtext[])
{
	...
	SendClientMessage(playerid,AQUAGREEN, "Vehiculo Cerrado '.");
	...
	SendClientMessage(playerid,AQUAGREEN, "Vehiculo Abierto '.");
	GetPlayerPos(playerid,X,Y,Z);
